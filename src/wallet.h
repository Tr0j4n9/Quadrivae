...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 QUAD at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zQUAD send
enum ZerocoinSpendStatus {
    ZQUAD_SPEND_OKAY = 0,                            // No error
    ZQUAD_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZQUAD_WALLET_LOCKED = 2,                         // Wallet was locked
    ZQUAD_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZQUAD_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZQUAD_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZQUAD_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZQUAD_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZQUAD_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZQUAD_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZQUAD_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZQUAD_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZQUAD_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZQUAD_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZQUAD_SPENT_USED_ZQUAD = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZQuadrivaeAutoBackups(bool fEnabled)
    {
    ...
    /// Get 1000QUAD output and keys which can be used for the Masternode
